...
async def process_with_agent(agent_type, user_input, user_id, session_id):
    ...
    Args:
        agent_type (str): Type of agent to use ("tech", "creative", "wisdom", etc.)
    ...
    while True:
        ...
        try:
            if choice == '1':
                ...
                if sub_choice == '1':
                    # Get primary agent for analysis
                    ...
                    profile = growth_engine.get_user_profile(user_id)
                    if not profile:
                        print("Not enough data to generate recommendations. Use JAY.AI more to build your profile!")
                        continue
                    
                    # Get domains that need attention
                    domain_levels = [(domain, data["level"]) for domain, data in profile["domains"].items()]
                    domain_levels.sort(key=lambda x: x[1])  # Sort by level
                    
                    focus_domains = [d[0] for d in domain_levels[:2]]  # Focus on lowest 2 domains
                    
                    system_prompt = f"""You're generating personalized recommendations for a user with the following profile:
                    - Current streak: {profile['streak']} days
                    - Areas needing attention: {', '.join(focus_domains)}
                    
                    Provide 3-5 actionable recommendations that:
                    1. Focus specifically on these domains: {', '.join(focus_domains)}
                    2. Are concrete and measurable
                    3. Build on their existing streak of {profile['streak']} days
                    4. Maintain JAY.AI's motivational, authentic voice
                    
                    Make these recommendations personalized, specific, and immediately actionable."""
                    
                    print("\nGenerating personalized recommendations... Please wait.")
                    recommendations = await primary_agent.process(
